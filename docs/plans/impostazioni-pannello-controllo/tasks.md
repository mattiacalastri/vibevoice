...
- [x] **T0 — Verifica la sovrapposizione col lavoro full-duplex** *(rischio dichiarato)*
...
- [x] **T1 — `config.py`: la chiave `vp`** *(D1)*
...
- [x] **T2 — `_start_engine()` passa `VIBEVOICE_VP`** *(D1)*
...
- [x] **T3 — `engine_restart_needed()` e la fine del riavvio a sproposito** *(D2)* ⭐
...
- [x] **T4 — History viva, con l'orario e lo svuotamento** *(D4, D5)*
...
- [x] **T5 — Layout a sezioni, finestra ridimensionabile** *(D3)*
...
Tutti i task chiusi tranne **T7** (verifica a occhio, gesto di Mattia).

Due difetti trovati **durante** l'esecuzione, non previsti dal piano:

1. **`save()` pretendeva tutte le chiavi.** Aggiungere `vp` a `DEFAULTS` ha armato un
   `KeyError` latente in `settingsChanged_`, che costruiva un dizionario parziale. Nessun
   test l'ha visto: quella callback è AppKit e non ne aveva. Riparato alla radice —
   `save()` ora è simmetrico a `load()`, così la prossima chiave non ripete la trappola.
2. **`_reload_history` azzerava tutto su una riga rotta.** Il `try` avvolgeva l'intero
   ciclo: una riga malformata cancellava anche le righe valide sopra di lei.

E una deviazione dichiarata dalla sequenza: la parte UI di T4 è stata fusa in T5, perché
T5 ricostruisce la finestra e farlo due volte sarebbe stato lavoro buttato.

**Prove**: `ruff` pulito · `pytest` 79 verdi (da 59: +20) · prova RED sul difetto madre
(reintrodurre il riavvio incondizionato rende rossi 4 test su 6) · `tools/smoke_settings_window.py`
20 controlli verdi sulla finestra reale.
